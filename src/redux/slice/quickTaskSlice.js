...
export const uniqueChecklistTaskData = createAsyncThunk(
  'fetch/checklistTask',
  async ({ page = 0, pageSize = 50, nameFilter = '', dateFilter = 'all', userFilter = 'all', append = false }) => {
    const result = await fetchChecklistData(page, pageSize, nameFilter, dateFilter, userFilter);
...
export const uniqueDelegationTaskData = createAsyncThunk(
  ...
  async ({ page = 0, pageSize = 50, nameFilter = '', dateFilter = 'all', userFilter = 'all', append = false }) => {
    const result = await fetchDelegationData(page, pageSize, nameFilter, dateFilter, userFilter);
    return { ...result, append };
  }
);
